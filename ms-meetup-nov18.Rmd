...
```{r osmdata-xml, eval = FALSE}
library (osmdata)
opq ("48155 de") %>%
    add_osm_feature (key = "highway") %>%
    osmdata_xml (file = "ms48155.osm")
```
...
```{r weight-streetnet-ms2, eval = FALSE, message = TRUE}
ms <- osm_poly2line (ms) # osmdata function
library (dodgr)
...
```{r dodgr-vertices1-fakey, eval = FALSE}
v <- dodgr_vertices (net) #<<
dim (v); head (v)
```
```{r dodgr-vertices1, eval = TRUE, echo = FALSE}
v <- dodgr_vertices (net)
dim (v); head (v, n = 5)
```

- `id` is the unique OSM is of each vertex<br>("node" in OSM terminology)
- most `dodgr` functions work either with<br>coordinates or `id` values
...
- To date, `dodgr` provides one of the only .bold[free] ways to perform such
  calcualtions, and is also entirely .bold[open source].
